        ...
        const [rows] = await dbTenant(`
            SELECT m.*,
                   COALESCE((
                       ...
                       WHERE ab.materialId = m.id
                         AND ab.blockType = 'MATERIAL'
...
export async function POST(req) {
    try {
        const session = await getSession();
        if (!session) return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
        const canAdd = await verifySessionPermission(session, "Materials", "Add");
        if (!canAdd) return NextResponse.json({ message: "Forbidden" }, { status: 403 });

        const body = await req.json();
        const data = materialSchema.parse(body);

        const materialId = await withTenantTransaction(async (tx) => {
            const { code } = await reserveSequentialCode(tx, {
                ...
                createParams: ["MATERIAL", "MAT", 1001, 4],
                whereKey: "entityType",
                whereValue: "MATERIAL",
                ...
                entityCodeField: "materialCode",
            });

            const [result] = await tx.execute(
                `INSERT INTO \`materials\` (materialCode, name, totalQuantity, costPerDay, status, remarks, createdAt, updatedAt)
                 VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`,
                [code, data.name, data.totalQuantity, data.costPerDay, data.status, data.remarks ?? null]
            );
            return result.insertId;
        });

        await logActivity("MATERIAL", materialId, "CREATE", `Created material: ${data.name}`);
        return NextResponse.json({ id: materialId, success: true }, { status: 201 });
    } catch (error) {
