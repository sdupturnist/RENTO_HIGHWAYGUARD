...
export default function MaterialReportPage() {
    ...
    const { data: reportData, isLoading: loading } = useQuery({
        queryKey: ["report-material", dateFrom?.toISOString(), dateTo?.toISOString(), customerId, projectId],
        queryFn: async () => {
            ...
            const res = await fetch(`/api/reports/material?${params.toString()}`);
    ...
    const exportColumns = [
        ...
        { key: "unit", header: "Unit", format: (v) => v || "-" },
    ...
    const handleExportPDF = async () => {
        try {
            ...
            generateReportPDF("Material Deployment Report", headers, exportedRows, `material-report-${format(new Date(), "yyyy-MM-dd")}`);
    ...
    const handleExportExcel = async () => {
        try {
            const { headers, rows: exportedRows } = formatDataForExport(rows, exportColumns);
            generateReportExcel("Material Deployment Report", headers, exportedRows, `material-report-${format(new Date(), "yyyy-MM-dd")}`);
    ...
    return (
        <div className="p-6 space-y-6">
            <ReportHeader
                ...
                description="Material usage by customer, project, and date range"
                backHref="/reports"
                        ...
                        <Table>
                            <TableHeader className="bg-slate-50/50 dark:bg-slate-900/50">
                                <TableRow className="hover:bg-transparent border-slate-200/60 dark:border-slate-800/60">
                                    <TableHead className="h-10">Material</TableHead>
                                    <TableHead className="h-10">Unit</TableHead>
                            ...
                            <TableBody>
                                {loading ? (
                                    ...
                                    <TableRow><TableCell colSpan={7} className="h-24 text-center">No material deployments found.</TableCell></TableRow>
                                ) : rows.map((row, idx) => (
