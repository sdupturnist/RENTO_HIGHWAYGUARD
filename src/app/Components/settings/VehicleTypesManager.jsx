...
export function VehicleTypesManager() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const url = editingType
                ? `/api/config/vehicle-types/${editingType.id}`
            ...
            if (res.ok) {
                toast.success(editingType ? "Vehicle type updated" : "Vehicle type created");
                setNewName("");
                setEditingType(null);
                setOpen(false);
                fetchTypes();
    ...
    const confirmDelete = async () => {
        if (!deleteId)
            return;
        try {
            const res = await fetch(`/api/config/vehicle-types/${deleteId}`, {
                method: "DELETE",
            });
            if (res.ok) {
                ...
                setNewName("");
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {editingType ? "Update the vehicle type name." : "Create a new category for vehicles (e.g., Sedan, SUV, Crane)."}
                            </DialogDescription>
                        </DialogHeader>
