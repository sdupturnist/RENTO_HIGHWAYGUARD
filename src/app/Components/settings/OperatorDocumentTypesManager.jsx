...
export function OperatorDocumentTypesManager() {
    ...
    // Form state: newName
    // Delete state is already handled by `handleDelete` taking ID, but loading state might be nice?
    // The previous implementation didn't have deleteId state, just window.confirm. I'll stick to that for now or upgrade?
    // I'll stick to the existing pattern for delete, but update creating/editing.
    const fetchTypes = async () => {
        try {
            const res = await fetch("/api/config/operator-doc-types");
    ...
    const handleSubmit = async () => {
        ...
        try {
            const url = editingType
                ? `/api/config/operator-doc-types/${editingType.id}`
                : "/api/config/operator-doc-types";
    ...
    const handleDelete = async (id) => {
        try {
            const res = await fetch(`/api/config/operator-doc-types/${id}`, {
                method: "DELETE",
            });
            if (res.ok) {
    ...
    return (<div className="space-y-4">
        <div className="flex justify-between items-center">
            <h3 className="text-lg font-medium">Operator Document Types</h3>
            <Dialog open={open} onOpenChange={(val) => {
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{editingType ? "Edit Operator Document Type" : "Add Operator Document Type"}</DialogTitle>
                        <DialogDescription>
                            {editingType ? "Update the document type name." : "Define a required document type for supervisors/operators (e.g. License, Visa)."}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        <Input placeholder="Document Name (e.g. Heavy License)" value={newName} onChange={(e) => setNewName(e.target.value)} />
