...
export function DetourTemplatePageForm({ initialData, currencySymbol = "AED" }) {
    const router = useRouter();
    const queryClient = useQueryClient();
    const isEdit = !!initialData;
    const formId = "detour-template-form";
    ...
    const form = useForm({
        ...
        defaultValues: isEdit ? {
            ...
            operatorCount: initialData.operatorCount ?? 0,
            bundleCostEnabled: !!initialData.bundleCostEnabled,
            bundleCostPerDay: Number(initialData.bundleCostPerDay ?? 0),
            status: initialData.status,
            remarks: initialData.remarks ?? "",
            requirements: (initialData.requirements || []).map((r) => ({
    ...
    const onSubmit = async (values) => {
        try {
            const url = isEdit ? `/api/detour-templates/${initialData.id}` : "/api/detour-templates";
    ...
    return (
        <Form {...form}>
            <form id={formId} onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <FormCard title="Template Details" description="Basic configuration for this detour service template.">
                    <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="vehicleCount" render={({ field }) => (
                                <FormItem>
                                    <FormLabel>Vehicle Slots</FormLabel>
                            ...
                            <FormField control={form.control} name="operatorCount" render={({ field }) => (
                                <FormItem>
                                    <FormLabel>Operator Slots</FormLabel>
                                    <FormControl><Input type="number" min="0" step="1" {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                ...
                <FormCard title="Bundle Billing" description="Invoice this detour service as a single bundled line item.">
                    <div className="space-y-4">
                        <FormField control={form.control} name="bundleCostEnabled" render={({ field }) => (
                            <FormItem className="flex items-center justify-between">
                                <FormLabel className="text-sm font-medium">Enable Bundle Billing</FormLabel>
                                <FormControl>
                        ...
                        {bundleCostEnabled && (
                            <FormField control={form.control} name="bundleCostPerDay" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input type="number" min="0" step="0.01" className="w-48" {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                        )}
                ...
                <FormCard title="Material & Labour Requirements" description="Resources automatically allocated with each deployment.">
                    <div className="space-y-3">
                        ...
                        {fields.length === 0 ? (
                            <p className="text-sm text-muted-foreground text-center py-6 border border-dashed rounded-xl">
                                No requirements defined. Click "Add Requirement" to add materials or labour.
                            </p>
                        ) : (
                            <div className="space-y-3">
                                <div className="grid grid-cols-[1fr_2fr_80px_36px] gap-2 text-xs font-medium text-muted-foreground px-1">
                                    <span>Type</span><span>Resource</span><span>Qty</span><span></span>
                                </div>
                                {fields.map((field, index) => {
                                    const resourceType = form.watch(`requirements.${index}.resourceType`);
                                    const options = getResourceOptions(resourceType);
                                    return (
                                        <div key={field.id} className="grid grid-cols-[1fr_2fr_80px_36px] gap-2 items-center">
